...
void CMyProgressCtrl::ResetProgressData(void)
{
	iPos = 0;
	iLower = 0;
	iUpper = 0;
...
DWORD CMyProgressCtrl::SetPos( DWORD nPos )
{
	DWORD oldpos = iPos;
...
void CMyProgressCtrl::SetRange( DWORD nLower, DWORD nUpper )
{	
	iLower			= nLower;
	iUpper			= nUpper;
...
DWORD CMyProgressCtrl::SetStep( DWORD nStep )
{
	DWORD oldstep = iStep;
	iStep = nStep;
	return (oldstep);
